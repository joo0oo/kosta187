...
public class ArrayExample4 {
	public static void main(String[] args) {
		...
		System.out.println(num); //입력받은 문자열 확인
		
		
		
		
		/* 과제!!
		 * 입력받은 num과 일치하는 계좌번호를 찾아서 
		 * "검색된 계좌는 다음과 같습니다" 하고 계좌 정보 출력
		 * 일치하는 계좌가 없는 경우도 만들기
		 */
		
		boolean isFind=false;
		for(int i=0; i<index; i++) {
			if( accounts[i].checkAccountNum(num) ) { 
				System.out.println("검색된 계좌는 다음과 같습니다..");
				accounts[i].print();
				isFind=true;
				break;
			}						
		...
		/*
		 * String 타입을 == 또는 != 연산자로 비교하면 그 String의 주소값이 비교됨 (레퍼런스 값끼리 비교됨)
		 * 객체 내용을 비교하려면 따로 메소드 사용해야 함 ->String은 equals() 함수 사용
		 */
	}
}
