...
public class Client extends Thread {
	...
	public void recieveMessage() {
		while(running) {
			...
			try {
				...
				System.out.println("[Debug] : "+nickName+" 클라이언트 수신 데이터: " + clientMessage);
		...
		if(socket != null) {
			try {
				socket.close();
				System.out.println(nickName+"socket closed");
			} catch (IOException e) {}
		}
	}
	...
	public void process(String message) {
		...
		case Protocol.CONNECT:
			...
			if(chatServer.isExistNickName(nickName)) {
				...
				chatServer.sendAllMessage(Protocol.GET_USERLIST+Protocol.DELEMETER+listToString(chatServer.getNickNames()));
				
				//sendMessage(Protocol.SET_MYNAME+Protocol.DELEMETER+nickName);
			}
			break;
		...
		case Protocol.DISCONNECT:
			...
			//chatServer.sendAllMessage(Protocol.DISCONNECT + Protocol.DELEMETER + nickName);
	...
	public String listToString(List<String> list) {
		...
		for (String string : list) {
			result += string+Protocol.DELEMETER;
		}
		return result;
	}
