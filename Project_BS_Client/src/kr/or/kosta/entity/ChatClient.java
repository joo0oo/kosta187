...
public class ChatClient {
	...
	public static final String SERVER="192.168.0.136";
	...
	public void waitingProcess(String[] tokens) {
		...
		case Protocol.ROOM_LIST: //생성된 방들의 목록 받기
			...
			for(int i=2; i<tokens.length;i=i+5) {
				...
				//hashMap에 추가
				ChatRoomInfo roomInfo=new ChatRoomInfo(Integer.parseInt(roomNum),title,Integer.parseInt(currentNum),Integer.parseInt(maxNum), roomMaster);
		...
		case Protocol.SC_ENTER_ROOM: // waiting에서 대화방으로 입장할때
			...
			case Protocol.SUCCESS: //입장 성공
				...
				frame.setSize(frame.WIDTH, frame.HEIGHT);
				frame.getChatRoomPanel().clearTA(); // 기존에 남아있었다면 삭제
				if (tokens.length>3) {
		...
		case Protocol.SC_NEW_ROOM: //새 대화방이 생겼을 때
			...
			case Protocol.SUCCESS: //방 생성 성공
				...
				frame.changeCard("chatRoom");
				frame.getChatRoomPanel().setRoomTitle(tokens[3]);
				frame.getChatRoomPanel().clearUserList();
				frame.getChatRoomPanel().clearTA();
				break;
		...
		case Protocol.SC_INVITE: //대기방에 있으면서 초대를 당했을 때
			...
			String roomMaster= tokens[roomInfoIndex+ 4];
			selectedRoomNum= Integer.parseInt(roomNum);
	...
	public void chattingProcess(String[] tokens) {
		...
		case Protocol.USER_LIST: //처음 방 들어왔을 때 기존 user들 목록 추가하기
			frame.getChatRoomPanel().clearUserList();
			frame.getChatRoomPanel().clearWhisperChoice();
			frame.getChatRoomPanel().addWhisperChoice("To All");
			for(int i=2; i<tokens.length;i++) {
		...
		case Protocol.EXIT_ROOM: //대화방에서 대기방으로 나갈 때
			frame.getChatRoomPanel().clearTA();
			frame.getChatRoomPanel().clearUserList();
			frame.getWaitingRoomPanel().clearWaitUserList();
			frame.getWaitingRoomPanel().clearRoomInfoTA(); 
